...
namespace NRA_ITAS
{
    public partial class reports : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string role = (string)Session["user_role"];
            if (role == "Teller")
            {
        ...
        protected void Recipts(object sender, EventArgs e)
        {
            ...
            try
            {
                string _ref = (string)Session["reference"];
                DataTable tb = dataconnect.getSelectedRow(_ref.Trim());
                DataTable dtt;
                DataTable dt = tb;
                foreach (DataRow r in dt.Rows)
                {
                    ...
                    string filens = Server.MapPath("~/Files/" + fileName);

                    string dts = (string)r["Date Of Payment"];
                    //string fileNames = string.Empty;
                    string taxPayerName = (string)r["Tax Payer Name"];
                    string prn = (string)r["PRN"];
                    string tin = (string)r["Tin"];
                    string t_amt1s = (string)r["Payment Amount"];
                    dtt = dataconnect.getSelectedRow_r(tin, prn);
                    ReceiptBuilder rb = new ReceiptBuilder();
                    HtmlToPdf converter = new HtmlToPdf();
                    //string t_amt1 = Convert.ToString(total_amt);

                    //if (File.Exists(filens))
                    //{
                    //    // Session["tt"].ToString();
                    //    File.Delete(filens);
                    ...
                    string res_ = rb.GenerateReceipt(dtt, fileName, taxPayerName, prn, tin, t_amt1s);
                    PdfDocument doc = converter.ConvertHtmlString(res_);

                    doc.Save(filens);
                    doc.Close();
                    //string pth = Server.MapPath(filens);
                    Response.ContentType = "application/pdf";
                    Response.AppendHeader("Content-Disposition", "attachment; filename=" + fileName);
                    Response.WriteFile(filens);
                    Response.Flush();
                    Response.End();
                    //}
                    //else
